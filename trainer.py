...
class Trainer:
    def __init__(self, model, CFG=None, loss=None, train_loader=None, val_loader=None, test_loader=None):
        self.CFG = CFG
    ...
    def _train_epoch(self, epoch):
        self.model.train()
        wrt_mode = 'train'
        self._reset_metrics()    #用于初始化或重置各种度量指标
        ...
        for img, gt in tbar:
            ...
            # 检查标签是否全为0或全为1
            if torch.all(gt == 0) or torch.all(gt == 1):
                tbar.update()
                continue  # 跳过当前迭代，执行下一次迭代
            ...
            if self.CFG.amp is True:
                ...
                pre = self.model(img)   #img: torch.Size([2, 1, 48, 48])   pre: torch.Size([2, 1, 48, 48])
                loss = self.loss(pre, gt)   #0.7628
                loss.backward()
                self.optimizer.step()
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for img, gt in tbar:
                ...
                self.total_loss.update(loss.item())
                self._metrics_update(
                    *get_metrics(predict, gt, threshold=self.CFG.threshold, running_auc=self.running_auc).values())
                tbar.set_description(
    ...
    def _metrics_update(self, auc, f1, acc, sen, spe, pre, iou):
        self.auc.update(auc)  #0.5297
        self.running_auc = self.auc.average
        self.f1.update(f1)  #0,1949
        self.acc.update(acc)  #0.3815
        self.sen.update(sen)    #0.707
        self.spe.update(spe)    #0.343
        self.pre.update(pre)    #0.113
        self.iou.update(iou)    #0.108

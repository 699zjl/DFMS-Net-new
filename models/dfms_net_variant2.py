...
class SpatialPathwayExtractor(nn.Module):
...
class TransformerTopologyInteraction(nn.Module):
    def __init__(self, in_channels):
        super(TransformerTopologyInteraction, self).__init__()
...
class SemanticAttentionAmplification(nn.Module):
    def __init__(self, in_channels):
        super(SemanticAttentionAmplification, self).__init__()
        self.gamma = nn.Parameter(torch.zeros(1))
        self.softmax = nn.Softmax(dim=-1)
...
class AffinityAttention(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.cab = SemanticAttentionAmplification(in_channels)
...
class DFMS_Net(nn.Module):
    def __init__(self, num_classes=1, num_channels=1, feature_scale=2, dropout=0.2, fuse=True, out_ave=True):
        ...
        self.spe3_1 = SpatialPathwayExtractor(filters[3], filters[3])
       ...
       # self.affinity_attention = AffinityAttention(128)
        self.spe40 = SpatialPathwayExtractor(filters[2], filters[2])
    ...
    def forward(self, x):
        ...
        x_down3_1 = self.spe3_1(x_down3_1)

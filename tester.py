...
class Tester(Trainer):
    def __init__(self, model, loss, CFG, checkpoint, test_loader, dataset_path, show=False):
        ...
        # 将模型移动到 GPU
        model = model.cuda()
        # 如果模型保存时使用了 DataParallel，这里也需要使用 DataParallel
        ...
        # 删除额外的键
        state_dict = {k.replace('module.', ''): v for k, v in checkpoint_state_dict.items() if "total_ops" not in k and "total_params" not in k}
        # 加载调整后的 state_dict
